...
export async function incrementDownloads(id, trackName) {
  ...
  if (res.rowCount > 0) {
    await logDownload(id, trackName); // Логируем только если инкремент прошел
    return res.rows[0]; // Возвращаем обновленного юзера
  }
  return null; // Возвращаем null если лимит превышен
}
