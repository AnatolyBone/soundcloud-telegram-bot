// db.js (ИСПРАВЛЕННАЯ И ПОЛНАЯ ВЕРСИЯ)
...
// === ИЗМЕНЕНИЕ №2: ДОБАВЛЕНА НЕДОСТАЮЩАЯ ФУНКЦИЯ ===
async function logEvent(userId, event) {
  try {
    const { error } = await supabase.from('events').insert([{ user_id: userId, event }]);
    if (error) {
      console.error(`❌ Ошибка логирования события "${event}" для userId ${userId} в Supabase:`, error.message);
    }
  } catch (e) {
    console.error(`❌ Критическая ошибка при вызове Supabase для logEvent:`, e.message);
  }
}
