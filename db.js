...
export async function findCachedTrack(soundcloudUrl) {
  try {
    const { data, error } = await supabase
      .from("track_cache")
      .select("file_id, track_name, info")
...
export async function cacheTrack(soundcloudUrl, fileId, title, info) {
  await pool.query(
    'INSERT INTO track_cache (soundcloud_url, telegram_file_id, title, info) VALUES ($1, $2, $3, $4) ON CONFLICT (soundcloud_url) DO UPDATE SET telegram_file_id = $2, title = $3, info = $4',
    [soundcloudUrl, fileId, title, info]
  );
}
