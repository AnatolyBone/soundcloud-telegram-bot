    ...
    app.post('/set-tariff', requireAuth, async (req, res, next) => {
        try {
            ...
            try {
                ...
                const htmlMessage = `🎉 Ваш тариф был изменен!\n\n` +
                `✨ Новый тариф: <b>${tariffName}</b>\n` +
                `⏳ Срок действия: <b>${parsedDays} дней</b>\n\n` +
                `Спасибо, что пользуетесь нашим ботом!`;
                await bot.telegram.sendMessage(userId, htmlMessage, { parse_mode: 'HTML' });
...
📣 Новости, фишки и бонусы в нашем канале 👉 <a href="https://t.me/SCM_BLOG">@SCM_BLOG</a>

💼 Тариф: <b>${tariffLabel}</b>
...
<code>${refLink}</code>
        ...
        if (!user.subscribed_bonus_used) {
            message += `\n\n🎁 <b>Бонус!</b>\nПодпишись на наш <a href="https://t.me/SCM_BLOG">новостной канал</a> и получи <b>7 дней тарифа Plus</b> бесплатно!`;
        }
        ...
        try {
            ...
            if (await isSubscribed(ctx.from.id, channel)) {
                ...
                await ctx.editMessageText(
                    ...
                    'Вам начислен бонус: <b>7 дней тарифа Plus</b>.\n\n' +
                ...
                await ctx.reply(`Пожалуйста, сначала подпишитесь на <a href="https://t.me/SCM_BLOG">${channel}</a>, а затем нажмите кнопку еще раз.`, {
                    parse_mode: 'HTML',
                    reply_markup: {
                        inline_keyboard: [
    ...
    bot.start(async (ctx) => {
        try {
            ...
            await ctx.replyWithHTML(messageText, { reply_markup: getBonusKeyboard(user) });
        } catch (e) {
    ...
    bot.hears(texts.menu, async (ctx) => {
        try {
            const user = ctx.state.user || await getUser(ctx.from.id);
            const messageText = formatMenuMessage(user, ctx);
            await ctx.replyWithHTML(messageText, { reply_markup: getBonusKeyboard(user), ...kb() });
        } catch (e) {
            await handleSendMessageError(e, ctx.from.id);
        }
    });
    ...
    bot.hears(texts.upgrade, async (ctx) => {
        try {
            // Используем HTML-парсер для этого сообщения тоже
            await ctx.replyWithHTML(texts.upgradeInfo.replace(/\*\*/g, '<b>').replace(/\n/g, '<br>'));
        } catch (e) {
            await handleSendMessageError(e, ctx.from.id);
        }
    });
