...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file && !file.mimetype.startsWith('audio/')) return cb(new Error('Только аудио-файлы'));
    cb(null, true);
  }
});
...
async function processUrlForIndexing(url) {
  ...
  try {
    ...
    // Если всё же пришёл плейлист — возьмём первый трек
...
async function startIndexer() {
  ...
  async function tick() {
    ...
    try {
      // Если у пользователей идут загрузки — даём приоритет им
      ...
      // Обрабатываем максимум 2 ссылки за цикл
      const batch = urls.slice(0, 2);
      for (const url of batch) {
        if (shuttingDown) break;
        await processUrlForIndexing(url);
        await new Promise(r => setTimeout(r, 5000)); // не душим event loop
      }
  ...
  // задержка старта, чтобы веб успел подняться
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      app.use(await bot.createWebhook({
        domain: WEBHOOK_URL,
        path: WEBHOOK_PATH,
        // secretToken: process.env.TG_WEBHOOK_SECRET, // опционально: если задашь в env
      }));
...
function setupTelegramBot() {
  const handleSendMessageError = async (error, userId) => {
    if (error.response?.error_code === 403) {
      ...
      console.error(`Ошибка при отправке сообщения для ${userId}:`, error.response?.description || error.message);
    }
  };
