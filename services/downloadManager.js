...
async function trackDownloadProcessor(task) {
    const { userId, url, trackName, trackId, uploader, playlistUrl, info } = task;
    ...
    try {
        ...
        if (info.filesize && info.filesize / (1024 * 1024) > TELEGRAM_FILE_LIMIT_MB) {
            throw new Error(`Трек слишком большой: ${trackName}. Размер: ${ (info.filesize / (1024 * 1024)).toFixed(2) } MB, лимит: ${TELEGRAM_FILE_LIMIT_MB} MB`);
        }

        await ytdl(url, {
            extractAudio: true, audioFormat: 'mp3', output: tempFilePath,
            embedMetadata: true,
            postprocessorArgs: [`-metadata artist="${info.uploader || 'SoundCloud'}"`, `-metadata title="${info.title}"`],
        ...
        if (sentMessage?.audio?.file_id) {
            ...
            await cacheTrack(url, sentMessage.audio.file_id, trackName, info);
        ...
        const errorMessage = `❌ Не удалось обработать трек: "${trackName}". Причина: ${err.message || err.stderr || 'Неизвестная ошибка'}`;
        if (statusMessage) {
        ...
        if (tasksToDownload.length > 0) {
            ...
            if (tasksToReallyDownload.length > 0) {
                ...
                for (const track of tasksToReallyDownload) {
                    downloadQueue.add({ userId, ...track, playlistUrl: isPlaylist ? url : null, priority: user.premium_limit, info: track.info });
