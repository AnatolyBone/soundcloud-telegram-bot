...
async function getTracksInfo(url) {
    const info = await pTimeout(ytdl(url, { dumpSingleJson: true, retries: CONFIG.YTDL_RETRIES, "socket-timeout": CONFIG.SOCKET_TIMEOUT }), {
        milliseconds: CONFIG.METADATA_FETCH_TIMEOUT_MS,
        message: 'Превышен таймаут получения метаданных.'
    });
    ...
    const tracks = isPlaylist
        ...
        : [{ url: info.webpage_url || url, trackName: sanitizeFilename(info.title), uploader: info.uploader || 'SoundCloud' }];
